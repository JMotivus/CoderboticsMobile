...
typedef NS_ENUM(NSUInteger, SDImageScaleMode) {
    /// Scale content to fit by changing aspect ratio if necessary
    SDImageScaleModeFill = 0,
    /// Scale content to fit while maintaining aspect ratio (may have transparent areas)
    SDImageScaleModeAspectFit = 1,
    /// Scale content to fill while maintaining aspect ratio (may clip content)
...
#else
/**
 * Rectangle corner options for platforms that don't have UIRectCorner
 */
...
/**
 * Provide common image transformation methods for `UIImage`.
 * Image processing is based on Core Graphics and vImage.
 */
 ...
 * @note The blend mode treat `self` as background image (destination), treat `tintColor` as input image (source). 
 *       So mostly you need `source` variant blend mode (use `sourceIn` not `destinationIn`), 
 *       which is different from UIKit's `+[UIImage imageWithTintColor:]`.
 * 
 * @param tintColor  The tint color.
...
#pragma mark - Image Color Analysis

/**
 * Return the pixel color at specify position. The point is from the top-left to the bottom-right and 0-based. 
 * The returned color is always in RGBA format. The image must be CG-based.
 *
 ...
 * @note The overhead of object creation means this method is best suited for infrequent color sampling. 
 *       For heavy image processing, grab the raw bitmap data and process yourself.
 *
...
/**
 * Return the pixel color array with specify rectangle. The rect is from the top-left to the bottom-right and 0-based. 
 * The returned color is always in RGBA format. The image must be CG-based.
 *
 * @note The rect's origin and size will be converted into integer.
 * @note The rect's width/height should not be smaller than or equal to 0. The minX/minY should not be smaller than 0. 
 *       The maxX/maxY should not be greater than width/height. 
 *       Attention this limit is different from `sd_colorAtPoint:` (point: (0, 0) like rect: (0, 0, 1, 1))
 * @note The overhead of object creation means this method is best suited for infrequent color sampling. 
 *       For heavy image processing, grab the raw bitmap data and process yourself.
 *
 ...
 * @return The new image with the CIFilter, or nil if an error occurs (e.g. no enough memory).
 */
