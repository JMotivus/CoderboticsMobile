 ...
 * Defines the required methods for memory cache implementations.
 */
...
/**
 * Create a new memory cache instance with the specified cache config.
 * You can check `maxMemoryCost` and `maxMemoryCount` used for memory cache.
 *
...
/**
 * A memory cache which auto purges the cache on memory warning and supports weak cache.
 * Extends NSCache with the SDMemoryCache protocol implementation.
 */
@interface SDMemoryCache <KeyType, ObjectType> : NSCache <KeyType, ObjectType> <SDMemoryCache>

/**
 * The configuration for this memory cache instance.
 * Read-only property that contains the configuration passed during initialization.
 */
